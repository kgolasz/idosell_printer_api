...
router.post('/fiskalizator/:id',function(req,res,next) {
    const request = {
        UniqueKey : printers.fiscals[req.params.id].key
    };
    // soap.createClientAsync( "http" + "://" + config.server.address + ":" + config.server.port )
    soap.createClient( 
        config.server.protocol + "://" + config.server.address + ":" + config.server.port + "/PrinterService",
        request,
        function(err,client) {
            if(err) {
                console.log(err);
                ...
                client.CheckFiskalizatorConnection(
                    request,
...
router.post('/printer/:id', function(req,res,next) {
    const request = {
